...
// Based on "isPrimeNumber_1",    only try the factors up to half of the input number.     take 1.43 sec to prove the example 1
...
// since all the prime numbers are odd except "2", so this time will only try the odd numbers and 2,   take 0.71 sec to prove the example 1
...
//This time only try the potential factors to sqrt(input number).  takes almost 0.0 sec for test example
